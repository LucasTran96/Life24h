...
import com.family.life24h.SQLite.tb_AverageSpeed;
import com.family.life24h.SQLite.tb_CurrentFamilyID;
import com.family.life24h.Utils.keyUtils;
import com.family.life24h.Utils.patternRegex;
...
import static com.family.life24h.Views.UIWellcome.UIUpgradePremium.ITEM_SKU_PREMINIUM_1_MONTH;
...
public class pre_Account implements imp_Account {
    ...
    public void updatePassword(String currentPassword, String newPassword, String passwordConfirm) {
        if (checkInputUpdatePassword(currentPassword, newPassword, passwordConfirm)) {
            WaitingDialog.showDialog(context);
            FirebaseUser user = objAccount.getCurrentUser();
            if (user != null) {
                user.updatePassword(newPassword)
                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                ...
                                if (task.isSuccessful()) {
                                    savePasswordToLocal(context, newPassword);
                                    mView.resultOfActionAccount(true, context.getResources().getString(R.string.Successfully_updated_the_password), keyUtils.updatePassword);
                                } else {
                                    mView.resultOfActionAccount(false, context.getResources().getString(R.string.Password_update_failed), keyUtils.updatePassword);
                                }
                            }
                        });
            }
    ...
    @Override
    public void checkTimeTrial() {
        long currentTime = Calendar.getInstance().getTimeInMillis();
        mDatabase.getReference().child(keyUtils.accountList).child(objAccount.getCurrentUser().getUid())
                .child(keyUtils.timeCreate)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.getValue() != null) {
                            long timeCreate = dataSnapshot.getValue(Long.class);
                            boolean a = currentTime - timeCreate >= keyUtils.TIME_TRIAL;

                            mView.resultCheckTimeTrial(currentTime - timeCreate >= keyUtils.TIME_TRIAL);
                        } else
                            mView.resultCheckTimeTrial(true);

                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError databaseError) {
                        mView.resultOfActionAccount(false, databaseError.getMessage(), keyUtils.checkTimeTrial);
                    }
                });
    }
    ...
    public void updateBought() {
        ...
        final BillingClient mBillingClient = BillingClient.newBuilder(context).setListener(new PurchasesUpdatedListener() {
            @Override
                                    ...
                                    if (responseCode == BillingClient.BillingResponse.OK
                                        ...
                                        for (Object skuDetailsObject : skuDetailsList) {
                                            ...
                                            if (ITEM_SKU_PREMINIUM_1_MONTH.equals(sku)) {
                                                mDatabase.getReference().child(keyUtils.accountList).child(objAccount.getCurrentUser().getUid())
                                                        .child(keyUtils.bought)
                                                        .setValue(queryPurchases(mBillingClient));
                                            }
                                        }
                                    }
                                }
