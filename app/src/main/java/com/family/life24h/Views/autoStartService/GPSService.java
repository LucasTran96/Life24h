...
import com.family.life24h.Models.objApplication.objLocation;
...
import com.family.life24h.Models.objectFirebase.family.fb_Safe_Area_Notification;
...
import com.family.life24h.SQLite.tb_Location;
import com.family.life24h.SQLite.tb_Message;
import com.family.life24h.Utils.keyUtils;
import com.family.life24h.Utils.notificationUtils;
...
import static com.family.life24h.Utils.keyUtils.IDFAMILY;
import static com.family.life24h.Utils.keyUtils.IDFamilyList;
import static com.family.life24h.Utils.keyUtils.LATITUDEOLDACCOUNT;
import static com.family.life24h.Utils.keyUtils.LONGTITUDEOLDACCOUNT;
import static com.family.life24h.Utils.keyUtils.Safe_Area_Notification;
import static com.family.life24h.Utils.keyUtils.location;
import static com.family.life24h.Utils.keyUtils.name;
...
import static com.family.life24h.Utils.notificationUtils.buildNotificationService;
import static com.family.life24h.Utils.timeUtils.getCurrentDate;
...
import static com.family.life24h.Views.UISafety.SpeedService.isDriving;

/*******************************************************************************
 *  Date created: 11/06/2019
 *  Last updated: 02/11/2020
 *  Name project: Life24h
 *  Description: AndroidGpsWatcherGL_Service is class service to assist in obtaining the position
 *  and check if the safety zone is within the safe zone or exit the safety zone.
 *  Auth: Lucas Walker & James Ryan
 *  History: 12/02/2020 update history location when internet offline.
 *  Auth: Lucas Walker
 ******************************************************************************/
...
public class GPSService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            pre_Safety.listenerEmergencyAssistance(this, new pre_Safety.interListenerEmergency() {
                @Override
                public void EmergencyAssistance(objAccount account, objEmergencyAssistance emergencyAssistance) {
                    final String title = getResources().getString(R.string.Emergency_assistance);
                    String message;

                    if(emergencyAssistance.getType().matches(keyUtils.TYPE_HELP)){
                        message = String.format(getResources().getString(R.string.announces_that_he_has_a_problem_to_help),
                                account.getName(),
                                account.getGender().toLowerCase().matches(keyUtils.MALE) ? getResources().getString(R.string.he) : getResources().getString(R.string.she),
                                account.getGender().toLowerCase().matches(keyUtils.MALE) ? getResources().getString(R.string.him) : getResources().getString(R.string.her));
                    }else{
                        message = String.format(getResources().getString(R.string.We_find_driving_speed_unusually_low),
                                account.getName(),
                                account.getGender().toLowerCase().matches(keyUtils.MALE) ? getResources().getString(R.string.him) : getResources().getString(R.string.her));
                    }

                    notificationUtils.buildNotificationEmergencyAssistance(GPSService.this,title,message);
                }
            });
